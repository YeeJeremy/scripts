...
set.seed(1)
r_index <- matrix(c(2, 3, 4, 2, 3, 4), ncol = 2)
sigma <- vol * sqrt(step)
n_disturb <- 20000
weight <- rep(1 / n_disturb, n_disturb)
disturb <- array(0, dim = c(4, 4, n_disturb))
disturb[1,1,] <- 1
...
n_path_grid <- 1000
grid_disturb <- array(0, dim = c(4, 4, n_path_grid, n_dec - 1))
grid_disturb[1,1,,] <- 1
...
## Generate paths
start <- c(1, 100, 100, 100)
path_sgrid <- PathDisturb(start, grid_disturb)
start_index <- rbind(c(1, 90, 90, 90), c(1, 100, 100, 100), c(1,110, 110, 110))
n_grid <- 2000
...
in_money <- apply(grid[,2:4], 1, max) >= strike
colIndex <- max.col(grid[,2:4]) + 1
reward <- array(0, dim = c(n_grid, 4, 2, 2, n_dec - 1))
...
scrap <- array(data = 0, dim = c(n_grid, 4, 2))
...
## Path disturbances 
set.seed(12345)
path_disturb <- array(0, dim = c(4, 4, n_path, n_dec - 1))
path_disturb[1, 1,,] <- 1
...
## Subsim disturbances
subsim <- array(0, dim = c(4, 4, n_subsim, n_path, (n_dec - 1)))
subsim[1,1,,,] <- 1
